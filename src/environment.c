...
int
environment_create (const char *name, tex_environment **env, int err)
{
...
int
environment_delete (tex_environment *env)
{
  if (env != NULL)
    {
      free (env->name);
...
int
environment_begin_write (const tex_environment *env, char *buffer,
                         size_t buffer_size)
{
...
int
environment_end_write (const tex_environment *env, char *buffer,
                       size_t buffer_size)
{
  if (env == NULL || buffer == NULL)
    {
...
int
environment_write (const tex_environment *env, char *buffer, size_t buffer_size)
{
  if (env == NULL || buffer == NULL)
    {
      return TEX_EINVAL;
